...
interface RawPDSEducation {
    level: PDSEducation["level"];
    school_name: string;
    degree_course?: string | null;
    period_from: string;
    period_to?: string | null;
    highest_level_attained?: string | null;
    year_graduated?: string | null;
    academic_awards?: string | null;
}

interface RawPDSEligibility {
...
interface RawPDSWorkExperience {
    date_from: string;
    date_to?: string | null;
    ...
    is_government_service?: boolean | number | null;
}

interface RawPDSVoluntaryWork {
    organization_name: string;
    organization_address: string;
    date_from: string;
    date_to?: string | null;
    number_of_hours: number | string;
    position: string;
}

interface RawPDSTraining {
...
interface RawPDSEntity {
    personalInfo: {
        ...
        residentialAddress?: RawPDSAddress | null;
        permanentAddress?: RawPDSAddress | null;
        contactInfo?: {
    ...
    familyBackground: {
        spouse?: {
            ...
            employee_business_name?: string | null;
        ...
        children?: RawPDSChild[];
    };
    education?: RawPDSEducation[];
    eligibility?: RawPDSEligibility[];
    workExperience?: RawPDSWorkExperience[];
    training?: RawPDSTraining[];
    voluntaryWork?: RawPDSVoluntaryWork[];
    otherInfo?: { info_type: string; information: string }[];
    references?: RawPDSReference[];
    employee: {
        employee_no: string;
...
export default function PDSOnboardPage() {
    ...
    const mapBackendPdsToFrontend = useCallback((data: RawPDSEntity): { formData: FullPDS; employeeMeta: { employeeNo: string; positionTitle: string; dateHired: string } } => {
        const { employee, personalInfo, familyBackground } = data;

        const formatDate = (dateValue: string | number | null | undefined) => {
            if (!dateValue || dateValue === "Present") return "";
            try {
        ...
        const mapAddress = (addr: RawPDSAddress | null | undefined): PDSAddress => ({
            houseBlockLot: String(addr?.house_no || ""),
            street: String(addr?.street || ""),
            subdivision: String(addr?.subdivision_village || ""),
            barangay: String(addr?.barangay || ""),
            cityMunicipality: String(addr?.city || ""),
            province: String(addr?.province || ""),
            zipCode: String(addr?.zip_code || ""),
        });

        const mappedPds: FullPDS = {
            personalInfo: {
                surname: personalInfo.surname,
                firstName: personalInfo.first_name,
                middleName: personalInfo.middle_name || "",
                nameExtension: personalInfo.name_extension || "",
                dateOfBirth: formatDate(personalInfo.date_of_birth),
                placeOfBirth: personalInfo.place_of_birth || "",
                sex: (personalInfo.sex || "Male") as FullPDS["personalInfo"]["sex"],
                civilStatus: (personalInfo.civil_status || "Single") as FullPDS["personalInfo"]["civilStatus"],
                height: String(personalInfo.height || ""),
                weight: String(personalInfo.weight || ""),
                bloodType: personalInfo.blood_type || "",
                gsisIdNo: personalInfo.governmentIds?.find(id => id.id_type === "GSIS")?.id_value || "",
                pagIbigIdNo: personalInfo.governmentIds?.find(id => id.id_type === "PAG_IBIG")?.id_value || "",
                philhealthNo: personalInfo.governmentIds?.find(id => id.id_type === "PHILHEALTH")?.id_value || "",
                sssNo: personalInfo.governmentIds?.find(id => id.id_type === "SSS")?.id_value || "",
                tinNo: personalInfo.governmentIds?.find(id => id.id_type === "TIN")?.id_value || "",
                agencyEmployeeNo: employee.employee_no || "",
                citizenship: personalInfo.citizenship || "",
                dualCitizenshipCountry: personalInfo.dual_citizenship_country || "",
                residentialAddress: mapAddress(personalInfo.residentialAddress),
                permanentAddress: mapAddress(personalInfo.permanentAddress),
                telephoneNo: personalInfo.contactInfo?.telephone_no || "",
                mobileNo: personalInfo.contactInfo?.mobile_no || "",
                email: personalInfo.contactInfo?.email_address || "",
            },
            familyBackground: {
                spouseSurname: familyBackground.spouse?.surname || "",
                spouseFirstName: familyBackground.spouse?.first_name || "",
                spouseMiddleName: familyBackground.spouse?.middle_name || "",
                spouseNameExtension: familyBackground.spouse?.name_extension || "",
                spouseOccupation: familyBackground.spouse?.occupation || "",
                spouseEmployerBusinessName: familyBackground.spouse?.employee_business_name || "",
                spouseBusinessAddress: familyBackground.spouse?.business_address || "",
                spouseTelephoneNo: familyBackground.spouse?.telephone_no || "",
                fatherSurname: familyBackground.father?.surname || "",
                fatherFirstName: familyBackground.father?.first_name || "",
                fatherMiddleName: familyBackground.father?.middle_name || "",
                fatherNameExtension: familyBackground.father?.name_extension || "",
                motherMaidenSurname: familyBackground.mother?.surname || "",
                motherFirstName: familyBackground.mother?.first_name || "",
                motherMiddleName: familyBackground.mother?.middle_name || "",
                children: familyBackground.children?.map((c) => ({
                    fullName: `${c.surname}, ${c.first_name}`,
                    dateOfBirth: formatDate(c.date_of_birth),
                    surname: c.surname,
                    firstName: c.first_name,
                    middleName: c.middle_name || "",
                    nameExtension: c.name_extension || "",
                })) || [],
            },
            education: data.education?.map((e) => ({
                ...
                periodFrom: formatDate(e.period_from),
                periodTo: formatDate(e.period_to || ""),
            ...
            civilServiceEligibility: data.eligibility?.map((e) => ({
                careerService: e.career_service,
                rating: e.rating,
                dateOfExamination: formatDate(e.date_of_examination),
                placeOfExamination: e.place_of_examination,
                licenseNumber: e.license_no || "",
                dateOfValidity: formatDate(e.date_of_validity || ""),
            })) || [],
            workExperience: data.workExperience?.map((w) => ({
                dateFrom: formatDate(w.date_from),
                dateTo: formatDate(w.date_to || "Present"),
            ...
            voluntaryWork: data.voluntaryWork?.map((v) => ({
                organizationName: v.organization_name,
                organizationAddress: v.organization_address,
                dateFrom: formatDate(v.date_from),
                dateTo: formatDate(v.date_to || ""),
                numberOfHours: String(v.number_of_hours),
                positionNatureOfWork: v.position,
            })) || [],
            learningDevelopment: data.training?.map((t) => ({
                title: t.training_title,
                dateFrom: formatDate(t.date_from),
                dateTo: formatDate(t.date_to),
                numberOfHours: String(t.number_of_hours),
                type: t.training_type,
                conductedSponsoredBy: t.conducted_by,
            })) || [],
            otherInfo: {
                specialSkillsHobbies: data.otherInfo?.filter(i => i.info_type === "special_skills").map(i => i.information) || [],
                nonAcademicDistinctions: data.otherInfo?.filter(i => i.info_type === "non_academic_recognitions").map(i => i.information) || [],
                membershipInAssociations: data.otherInfo?.filter(i => i.info_type === "organization_memberships").map(i => i.information) || [],
            },
        ...
        return { formData: mappedPds, employeeMeta: meta };
    }, []);
        ...
        async function loadExisting() {
            ...
            try {
                ...
                if (env.success && env.data && env.data.personalInfo && env.data.employee) {
                    const { formData: mapped, employeeMeta: mappedMeta } = mapBackendPdsToFrontend(env.data);
                    setFormData(mapped);
    ...
    const copyResidentialToPermanent = () => {
        setFormData((prev) => ({
            ...
            personalInfo: {
                ...prev.personalInfo,
                permanentAddress: { ...prev.personalInfo.residentialAddress },
            },
        }));
    };

    const updatePersonal = (field: keyof PDSPersonalInfo, value: string | number | boolean | null | object) => {
        setFormData((prev) => ({
            ...prev,
    ...
    const updateFamily = (field: keyof PDSFamilyBackground, value: string | number | boolean | null | object) => {
        setFormData((prev) => ({
            ...prev,
    ...
    const addChild = () => {
        setFormData((prev) => ({
            ...
            familyBackground: {
                ...
                children: [...prev.familyBackground.children, emptyChild()],
            },
        }));
    };
    ...
    const updateChild = (index: number, field: keyof PDSChild, value: string | number | boolean | null) => {
        setFormData((prev) => {
            const children = [...prev.familyBackground.children];
            children[index] = { ...children[index], [field]: value };
            return {
                ...prev,
                familyBackground: { ...prev.familyBackground, children },
            };
        });
    };

    const updateEducation = (index: number, field: keyof PDSEducation, value: string | number | boolean | null) => {
        setFormData((prev) => {
    ...
    const addEducation = () => {
        setFormData((prev) => {
            // Find appropriate next level to add or just add default
            return {
                ...prev,
                education: [...prev.education, emptyEducation("College")],
            };
        });
    };
    ...
    const updateEligibility = (index: number, field: keyof PDSCivilServiceEligibility, value: string | number | boolean | null) => {
        setFormData((prev) => {
            const eligibility = [...prev.civilServiceEligibility];
            eligibility[index] = { ...eligibility[index], [field]: value };
            return { ...prev, civilServiceEligibility: eligibility };
        });
    };
    ...
    const addEligibility = () => {
        setFormData((prev) => ({
            ...prev,
            civilServiceEligibility: [...prev.civilServiceEligibility, emptyEligibility()],
        }));
    };
    ...
    const updateWork = (index: number, field: keyof PDSWorkExperience, value: string | number | boolean | null) => {
        setFormData((prev) => {
            const work = [...prev.workExperience];
            work[index] = { ...work[index], [field]: value };
            return { ...prev, workExperience: work };
        });
    };
    ...
    const updateLD = (index: number, field: keyof PDSLearningDevelopment, value: string | number | boolean | null) => {
        setFormData((prev) => {
            const ld = [...prev.learningDevelopment];
            ld[index] = { ...ld[index], [field]: value };
            return { ...prev, learningDevelopment: ld };
        });
    };
    ...
    const addLD = () => {
        setFormData((prev) => ({
            ...prev,
            learningDevelopment: [...prev.learningDevelopment, emptyLD()],
        }));
    };

    const removeLD = (index: number) => {
        setFormData((prev) => ({
            ...prev,
            learningDevelopment: prev.learningDevelopment.filter((_, i) => i !== index),
        }));
    };

    const updateVoluntary = (index: number, field: keyof PDSVoluntaryWork, value: string | number | boolean | null) => {
        setFormData((prev) => {
            const v = [...prev.voluntaryWork];
            v[index] = { ...v[index], [field]: value };
            return { ...prev, voluntaryWork: v };
        });
    };
    ...
    const addVoluntary = () => {
        setFormData((prev) => ({
            ...prev,
            voluntaryWork: [...prev.voluntaryWork, emptyVoluntary()],
        }));
    };

    const removeVoluntary = (index: number) => {
        setFormData((prev) => ({
            ...prev,
            voluntaryWork: prev.voluntaryWork.filter((_, i) => i !== index),
        }));
    };

    const updateOtherItem = (field: keyof FullPDS["otherInfo"], index: number, value: string) => {
        setFormData((prev) => {
            const list = [...prev.otherInfo[field]];
            list[index] = value;
            return {
                ...prev,
                otherInfo: { ...prev.otherInfo, [field]: list },
            };
        });
    };

    const addOtherItem = (field: keyof FullPDS["otherInfo"]) => {
        setFormData((prev) => ({
            ...prev,
            otherInfo: {
                ...prev.otherInfo,
                [field]: [...prev.otherInfo[field], ""],
            },
        }));
    };

    const removeOtherItem = (field: keyof FullPDS["otherInfo"], index: number) => {
        setFormData((prev) => ({
            ...prev,
            otherInfo: {
                ...prev.otherInfo,
                [field]: prev.otherInfo[field].filter((_, i) => i !== index),
            },
        }));
    };

    const updateReference = (index: number, field: keyof PDSReference, value: string | number | boolean | null) => {
        setFormData((prev) => {
    ...
    const updateGovId = (field: keyof PDSGovernmentIssuedID, value: string | number | boolean | null) => {
        setFormData((prev) => ({
            ...prev,
            governmentIssuedId: { ...prev.governmentIssuedId, [field]: value },
        }));
    };
                                ...
                                <tbody>
                                    {formData.education.filter((e) => e.schoolName).map((e, i) => (
                                        <tr key={i} className="border-b border-stone-100 text-stone-700">
                                            ...
                                            <td className="py-2 px-2">{e.periodFrom} 
 {e.periodTo}</td>
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-xs">
                                ...
                                <tbody>
                                    {formData.workExperience.map((w, i) => (
                                        <tr key={i} className="border-b border-stone-100 text-stone-700">
                                            <td className="py-2 px-2">{w.dateFrom} 
 {w.dateTo}</td>
                        ...
                        <div className="space-y-2 text-sm">
                            {formData.learningDevelopment.map((ld, i) => (
                                <div key={i} className="p-3 bg-stone-50/80 rounded-xl text-stone-700">
                                    ...
                                    <span className="ml-2 text-stone-500">
                                        ({ld.dateFrom} 
 {ld.dateTo}, {ld.numberOfHours} hrs)
                                    </span>
                                </div>
                            ))}
                        </div>
    ...
    if (isLoadingExisting) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[400px] gap-3">
                ...
                <p className="text-stone-500 font-medium">Checking onboarding status...</p>
            </div>
        );
    }
    if (hasExistingRecord) {
        return (
            <div className="max-w-2xl mx-auto mt-20 p-12 bg-white rounded-[2.5rem] border border-stone-200 text-center shadow-xl shadow-stone-200/50 transition-all animate-in fade-in zoom-in duration-500">
                <div className="w-20 h-20 bg-green-50 rounded-full flex items-center justify-center mx-auto mb-6">
                    <CheckCircle2 className="w-10 h-10 text-green-600" />
                </div>
                <h2 className="text-3xl font-bold text-stone-900 mb-4 font-outfit">Onboarding Completed</h2>
                <p className="text-stone-600 mb-10 text-lg leading-relaxed">
                    You have already submitted your Personal Data Sheet. To ensure data integrity, the onboarding portal is now locked. You can view your current records in your profile.
                </p>
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                    <a
                        href="/employee/my-pds"
                        className="px-8 py-4 bg-stone-900 text-white rounded-2xl font-semibold hover:bg-stone-800 transition-all shadow-lg shadow-stone-900/20 active:scale-95"
                    >
                        View My PDS
                    </a>
                    <a
                        href="/employee/dashboard"
                        className="px-8 py-4 bg-stone-100 text-stone-600 rounded-2xl font-semibold hover:bg-stone-200 transition-all active:scale-95"
                    >
                        Back to Dashboard
                    </a>
                </div>
