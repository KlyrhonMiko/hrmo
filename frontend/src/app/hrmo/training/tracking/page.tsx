...
interface TrainingTrackingPageProps {
    userRole?: Role;
}

export default function TrainingTrackingPage({ userRole = "HR Head" }: TrainingTrackingPageProps) {
    ...
    const filtered = useMemo(() => {
        return trainings.filter((t) => {
            ...
            const matchSearch =
                !q ||
                (t.training_title || t.title || "").toLowerCase().includes(q) ||
                t.participants.some((p) => p.name.toLowerCase().includes(q));
            const matchType = !filterType || (t.training_type || t.type) === filterType;
    ...
    function openEditModal(training: TrainingWithParticipants) {
        ...
        setForm({
            title: training.training_title || training.title || "",
            type: (training.training_type || training.type || "Seminar") as TrainingRecord["type"],
            conductedBy: training.conducted_by || training.conductedBy || "",
            venue: training.venue || "",
            dateFrom: training.date_from || training.dateFrom || "",
            dateTo: training.date_to || training.dateTo || "",
            numberOfHours: Number(training.number_of_hours) || training.numberOfHours || 0,
            status: (training.status || "Upcoming") as TrainingRecord["status"],
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="flex flex-wrap gap-x-5 gap-y-1 text-[12.5px] text-stone-500">
                                            <span className="inline-flex items-center gap-1">
                                                ...
                                                {tr.conducted_by || tr.conductedBy}
                                            </span>
                                            <span className="inline-flex items-center gap-1">
                                            ...
                                            <span className="inline-flex items-center gap-1">
                                                ...
                                                {formatDate(tr.date_from || tr.dateFrom || "")}
                                                {(tr.date_from || tr.dateFrom) !== (tr.date_to || tr.dateTo) &&
                                                    ` - ${formatDate(tr.date_to || tr.dateTo || "")}`}
                                            </span>
                                            <span className="inline-flex items-center gap-1">
